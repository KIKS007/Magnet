...
////// Emissive:
                ...
                float3 node_8050 = lerp(_Scanlines_var.rgb,dot(_Scanlines_var.rgb,float3(0.3,0.59,0.11)),_DesaturationAmount);
                ...
                float node_5012 = (pow(frac((1.0 - ((float2(i.uv0.r,i.uv0.g)*float2(1.0,_ScanlineDensity))+(float2(0.0,_ScanlineSpeed)*node_5120.r))).g),_ScanlineExp)*_ScanlineOpacity);
                float node_6413 = (_Opacity+node_5012);
                float3 emissive = ((((((_Texture_var.rgb*_Color.rgb)*(_ScanlinesIntensity+node_8050))*lerp(1.0,((node_2329*0.5+0.5)*_NoisePower),_NoisePower))*_Brightness)+node_5012)*node_6413);
